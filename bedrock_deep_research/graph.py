...
from .nodes import (ArticleHeadImageGenerator, ArticleOutlineGenerator,
                    CompletedSectionsFormatter, FinalSectionsWriter,
                    HumanFeedbackProvider, InitialResearcher,
                    SectionSearchQueryGenerator, SectionWebResearcher,
                    SectionWriter, compile_final_article,
...
class BedrockDeepResearch:
    ...
    def __create_workflow(self):
        builder = StateGraph(ArticleState, input=ArticleInputState,
                             output=ArticleOutputState, config_schema=Configuration)
        ...
        builder.add_conditional_edges(
            CompletedSectionsFormatter.N, initiate_final_section_writing, [FinalSectionsWriter.N])
    ...
    async def start(self, topic: str):
        ...
        return await self.graph.ainvoke({"topic": topic}, self.config, stream_mode="updates")
    ...
    async def feedback(self, feedback):
        ...
        return await self.graph.ainvoke(Command(resume=feedback), self.config, stream_mode="updates")
