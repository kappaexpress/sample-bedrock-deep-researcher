...
class FinalSectionsWriter:
    ...
    def __call__(self, state: SectionState, config: RunnableConfig):
        ...
        section.content = self._generate_final_sections(
            writer_model, final_section_writer_instructions, section, completed_report_sections)

        return {"completed_sections": [section]}

    @exponential_backoff_retry(Exception, max_retries=10)
    def _generate_final_sections(self, model: ChatBedrock, system_prompt: str, section: Section, completed_report_sections: str) -> str:
        # Format system instructions
        system_instructions = system_prompt.format(
            section_title=section.name, section_description=section.description, context=completed_report_sections)

        # Generate section
        section_content = model.invoke([SystemMessage(content=system_instructions)] +
                                       [HumanMessage(content="Generate a section of an article based on the provided sources.")])
