...
class SectionWriter:
    ...
    def __call__(self, state: SectionState, config: RunnableConfig) -> Command[Literal[END, SectionWebResearcher.N]]:
        ...
        section.content = self._generate_section_content(
            writer_model, section_writer_instructions, section, source_str, writing_guidelines)
    ...
    def _generate_section_content(self, model: ChatBedrock, system_prompt: str, section: Section, search_content: str, writing_guidelines: str) -> str:
        system_prompt = system_prompt.format(
            section_title=section.name, section_topic=section.description, context=search_content, section_content=section.content, writing_guidelines=writing_guidelines)
    ...
    @exponential_backoff_retry(Exception, max_retries=10)
    def _grade_section_content(self, model: ChatBedrock, system_prompt: str, section: Section) -> Feedback:
        ...
        feedback = structured_llm.invoke([SystemMessage(content=section_grader_instructions_formatted)]+[
            HumanMessage(content="Grade the article and consider follow-up questions for missing information:")])
